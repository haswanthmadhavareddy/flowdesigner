...
const COLORS = {
  ...
  accent: "#0A6ED1",
  accentAlt: "#7B61FF",
  accentGlow: "rgba(10,110,209,0.22)",
  primary: "#0A6ED1",
  text: "#F5F7FA",
  textDim: "#AEB7C2",
  muted: "#AEB7C2",
...
const Badge = ({ children, color = COLORS.accent, bg, style, className }) => (
  <span className={className} style={{
    display:"inline-flex", alignItems:"center", justifyContent:"center", padding:"10px 18px", borderRadius:999,
    fontSize:14, fontWeight:700, letterSpacing:"0.08em",
    background: bg || `${color}22`, color, border:`1px solid ${color}44`,
    boxShadow: `0 0 24px ${color}22`, transition:"all .25s ease", ...style
  }}>{children}</span>
);
  ...
  return (
    <div
      ...
      style={{
        ...
        border:`1px solid ${hov ? "rgba(10,110,209,0.25)" : COLORS.border}`,
        borderRadius:20,
        transition:"all .25s ease",
        cursor: onClick ? "pointer" : "default",
        boxShadow: hov ? `0 20px 50px rgba(0,0,0,0.16)` : "0 10px 30px rgba(0,0,0,0.08)",
...
const Input = ({ label, value, onChange, placeholder, type="text", required }) => (
  <div style={{ marginBottom:16 }}>
    <label style={{ display:"block", fontSize:12, fontWeight:600, color:COLORS.textDim, marginBottom:6, letterSpacing:"0.08em" }}>
    ...
    <input
      ...
      style={{
        width:"100%", padding:"14px 16px", background:"rgba(255,255,255,0.04)", border:`1px solid ${COLORS.border}`,
        borderRadius:16, color: value ? COLORS.text : COLORS.textDim, fontSize:14, fontWeight:500, outline:"none", boxSizing:"border-box",
        transition:"border .2s, transform .2s",
      }}
      onFocus={e => { e.target.style.borderColor = COLORS.accent; e.target.style.transform = "translateY(-1px)" }}
      onBlur={e => { e.target.style.borderColor = COLORS.border; e.target.style.transform = "none" }}
    />
  </div>
);
        ...
        return (
          <div key={s.id} className="stepper-step">
            <div className="stepper-step-inner">
              <div className={`stepper-badge ${completed ? 'completed' : active ? 'active' : 'inactive'}`}>
                {completed ? '✓' : s.id}
              </div>
              <span className={`stepper-label ${active ? 'active' : ''}`}>{s.label}</span>
            </div>
            {i < STEPS.length - 1 && (
    ...
    <div>
      ...
      <div style={{ position:"relative", marginBottom:12 }}>
        <textarea
          ...
          style={{ borderColor: text ? COLORS.accent : COLORS.border }}
          onFocus={e => e.target.style.borderColor = COLORS.accent}
...
export default function App() {
  ...
  return (
    <div className="app-shell" style={{ minHeight:"100vh", fontFamily:"72, 'Inter', sans-serif", color:COLORS.text, overflowX:"hidden" }}>
          ...
          <div className="hero-right">
            <div className="hero-text-block">
              <div className="hero-title">
                <span>FLOW</span>
                <span className="hero-gradient">DESIGNER</span>
              </div>
              <p className="hero-subtitle">AI-Powered Integration Composer for SAP Integration Suite</p>
            </div>
            <div className="chip-row">
              <Badge className="chip-pill" color={COLORS.primary} bg="rgba(10,110,209,0.14)">SAP BTP</Badge>
              <Badge className="chip-pill" color={COLORS.accentAlt} bg="rgba(139,92,246,0.14)">Joule AI</Badge>
            </div>
            <div className="feature-grid">
              <div className="feature-card">
                <span>Experience</span>
                <span>Enterprise-grade SAP integration design</span>
              </div>
              <div className="feature-card">
                <span>Outcome</span>
                <span>Fiori Horizon inspired clarity and control</span>
              </div>
            </div>
          </div>
        </div>
